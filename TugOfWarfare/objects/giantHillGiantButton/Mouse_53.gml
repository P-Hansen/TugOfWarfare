...
if(clicked == true and global.goldTotal	>= price and !place_meeting(_x, _y, buildingClass) and _y < room_height-100 and _x < room_width/2 and !place_meeting(_x, _y, ponds)){
	...
	*/
	...
	//killing grass
	if(collision_rectangle(_x-48, _y-48, _x+48, _y+48, [flammableClass, grassBurnt, treeBurnt, tree1], false, true) != noone){
		var targets = ds_list_create();
		var count = collision_rectangle_list(_x-48, _y-48, _x+48, _y+48, [flammableClass, grassBurnt, treeBurnt, tree1], false, true, targets, false);
		show_debug_message(_x);
		show_debug_message(targets);
		for (var i = 0; i < count; i++) {
			var target = ds_list_find_value(targets, i);
